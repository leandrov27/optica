...
export default function useDeleteEvent({ itemsInPage }: useDeleteEventProps) {
  ...
  const handleDeleteEvent = async (id: number) => {
    setIsDeleting(true);
    try {
      const resp = await ax.delete(API_ENDPOINTS.admin.event.delete(id));
      ...
      if (itemsInPage === 1 && currentPage > 1) {
        ...
        toast.success(`Evento #${id} eliminado exitosamente. Redirigiendo a página ${newPage}`);
      } else {
        // Si no es necesario cambiar de página, solo refrescar
        router.refresh();
        toast.success(`Evento #${id} eliminado exitosamente`);
      }
