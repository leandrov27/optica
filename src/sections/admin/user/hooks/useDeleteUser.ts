...
export default function useDeleteUser({ itemsInPage }: useDeleteUserProps) {
    ...
    const handleDeleteUser = async (id: number) => {
        setIsDeleting(true);
        try {
            const resp = await ax.delete(API_ENDPOINTS.admin.user.delete(id));
            ...
            if (itemsInPage === 1 && currentPage > 1) {
                ...
                toast.success(`Usuario #${id} eliminado exitosamente. Redirigiendo a página ${newPage}`);
            } else {
                // Si no es necesario cambiar de página, solo refrescar
                router.refresh();
                toast.success(`Usuario #${id} eliminado exitosamente`);
            }
