...
export default function useDeleteProduct({ itemsInPage }: useDeleteProductProps) {
    ...
    const handleDeleteProduct = async (id: number) => {
        setIsDeleting(true);
        try {
            const resp = await ax.delete(API_ENDPOINTS.admin.product.delete(id));
            ...
            if (itemsInPage === 1 && currentPage > 1) {
                ...
                toast.success(`Producto #${id} eliminado exitosamente. Redirigiendo a página ${newPage}`);
            } else {
                // Si no es necesario cambiar de página, solo refrescar
                router.refresh();
                toast.success(`Producto #${id} eliminado exitosamente`);
            }
