...
export default function ProductNewEditDialog({
  product,
  openDialog = false,
  onCloseDialog,
}: ProductNewEditDialogProps) {
    ...
    >
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <DialogTitle textAlign="left" sx={{ py: 1.5 }}>
          {isEdit ? 'Modificar' : 'Registrar'} Producto
        </DialogTitle>
        ...
        <DialogContent sx={{ p: 2 }}>
          <Grid container spacing={2}>
            <Grid xs={12} md={12} lg={12}>
              {product?.category && (
                <Stack flexDirection="row" spacing={1} sx={{ mb: 0.5 }}>
                  <Typography variant="subtitle2">Categoría actual:</Typography>
              ...
              >
                ...
                <Button
                  ...
                  sx={{ minWidth: 20, mt: 1.5, flexShrink: 0 }}
                  variant="contained"
                  size="small"
                >
            ...
            <Grid xs={12} md={12} lg={12}>
              {product?.satCode && (
                <Stack flexDirection="row" spacing={1} sx={{ mb: 0.5 }}>
                  <Typography variant="subtitle2">Código SAT actual:</Typography>

                  <Label color="info" style={{ textTransform: 'none' }}>
                    {product.satCode.codeSat} - {product.satCode.name}
                  </Label>
                </Stack>
              )}
            ...
            <Grid xs={4} md={4} lg={4}>
              <RHFTextField name="code" label="Código" disabled={isSubmitting} />
            </Grid>

            <Grid xs={8} md={8} lg={8}>
              <RHFTextField name="price" label="Precio" disabled={isSubmitting} />
            </Grid>

            <Grid xs={12} md={12} lg={12}>
              <RHFTextField name="description" label="Descripción" disabled={isSubmitting} />
            </Grid>
      ...
      <SatCodeNewDialog openDialog={satDialog.value} onCloseDialog={satDialog.onFalse} />
    </Dialog>
  );
}
