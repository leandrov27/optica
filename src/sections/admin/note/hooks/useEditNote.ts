...
export default function useEditNote({ note }: useEditNoteProps) {
  ...
  const methods = useForm({
    ...
    defaultValues: {
      clientId: note.clientId || 0,
      deliveryDate: note.deliveryDate || null,
      ...
      noteDetails: note.noteDetails.map(d => ({
        ...d,
        finalPrice: d.finalPrice ?? d.unitPrice,
      })),
    },
  });
  ...
  const handleDetailChange = useCallback((index: number, field: IEditableField, value: string) => {
    ...
    const newFinalPrice = field === 'finalPrice' ? numericValue : Number(currentItem.finalPrice);
  ...
  // --- SUBMIT ---
  const onSubmit = useCallback(async (formValues: IUpdateNotePayload) => {
    try {
      const resp = await ax.post(API_ENDPOINTS.admin.note.update(note.id), formValues);
