...
export async function POST(request: Request) {
  ...
  try {
    ...
    const totalPaid = saleNote.payments.reduce((sum, payment) => sum + Number(payment.amount), 0);
    ...
    if (newPendingBalance <= 0.001) {
      // Si el nuevo saldo es 0 o negativo (con tolerancia)
    ...
    if (error.code === 'P2025') {
      return NextResponse.json({ message: 'La nota de venta no fue encontrada' }, { status: 404 });
    }
