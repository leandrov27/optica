...
//
import { CFDISchema, PaymentMethodSchema, TaxRegimeSchema, TypeSchema } from './sub-schemas';

// ----------------------------------------------------------------------

const rfcBaseRegex =
  /^([A-ZÑ&]{3,4})([0-9]{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12][0-9]|3[01]))([A-Z\d]{2})([A\d])$/;
const RFC_GENERIC = 'XAXX010101000';

export const DiagnosisHistory = z
  .object({
...
const ClientSchema = z.object({
  ...
  phone: z
    ...
    .refine((value: string) => isPhoneValid(value), {
      error: 'Formato inválido para el país seleccionado',
    })
    .trim(),
  type: TypeSchema,
  observations: z.string().toLowerCase().trim().nullable(),
  //^ --------
  //^ TAX INFO
  //^ --------
  rfc: z
  ...
  postalCode: z
    ...
    .transform((val) => (val === '' ? null : val))
    .nullable(),
  cfdiUse: CFDISchema.nullable(),
  taxRegime: TaxRegimeSchema.nullable(),
  paymentMethod: PaymentMethodSchema.nullable(),
  address: z.string().toLowerCase().trim().nullable(),
  //~ --------------
  //~ DIAGNOSIS INFO
  //~ --------------
  date: z.any().nullable(),
  leftAxis: z.string().trim(),
  leftSphere: z.string().trim(),
  leftCylinder: z.string().trim(),
  di: z.string().trim(),
  rightAxis: z.string().trim(),
  rightSphere: z.string().trim(),
  rightCylinder: z.string().trim(),
  addition: z.string().trim(),
  add: z.string().trim(),
  addi: z.string().trim(),
  notes: z.string().toLowerCase().trim(),
  diagnoses: z.array(DiagnosisHistory),
});

// ----------------------------------------------------------------------

export const CreateUpdateClientSchema = ClientSchema.omit({ id: true })
  .extend({
  ...
  .check((ctx) => {
    ...
    if (enableTaxInfo && !value.billingEmail) {
      issues.push({
        ...
        path: ['billingEmail'],
        message:
          'El correo de facturación es requerido cuando la información fiscal está habilitada',
        input: [billingEmail],
      });
    }
